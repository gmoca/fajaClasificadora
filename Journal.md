...
- **Corrección de Bucle Infinito en UART TX (system.c):** Encontré que la rutina de interrupción de baja prioridad `isr_low()` llamaba a `uart_isr_handler()` únicamente cuando `RCIF` estaba activo, pero omitía procesar la transmisión (`TXIF`). Al iniciarse el envío del primer carácter, se habilitaba `TXIE = 1`, lo que disparaba la interrupción inmediatamente. Al no ser manejada en `isr_low()`, el microcontrolador volvía a entrar al vector de interrupción de forma instantánea al salir, quedando atrapado en un bucle infinito que congelaba la ejecución del CPU en la primera letra del buffer.
- **Corrección de Prioridad de Interrupciones (TXIP y TMR0IP):** Los registros `TXIP` (prioridad de TX de UART) y `TMR0IP` (prioridad de desbordamiento de Timer 0) tienen por defecto el valor `1` (Alta Prioridad) al encender el PIC18F4550. Dado que el software los maneja dentro de `isr_low()` (Baja Prioridad), estas interrupciones se desviaban a `isr_high()` (donde no eran procesadas ni limpiadas), atascando al microcontrolador en un bucle eterno de alta prioridad 0.5ms después de iniciar la simulación (cuando TMR0 desbordaba por primera vez). Se agregaron de forma explícita `IPR1bits.TXIP = 0;` en `uart.c` e `INTCON2bits.TMR0IP = 0;` en `system.c` para forzarlos a baja prioridad.
- **Resolución de Bloqueo de Bus I2C y Corrección de Dirección de Pantalla (`firmware/lcd.h` y `main.c`):** 
...
- **Resolución de Error Sintáctico TCSS y Botones Recortados en Móvil:** Dado que Textual CSS no soporta la directiva `@media` de CSS estándar, implementé responsividad a través del método `on_resize` en Python (añadiendo/removiendo la clase `.wide-screen`). 
  - Corregí un bug crítico de clipping lateral en celulares: al estar en formato horizontal (`Horizontal`), el quinto botón de la barra de control (**"Modo TEST"**) se recortaba fuera del borde derecho de la pantalla y era inaccesible. Ahora, los botones de control (`#dash-controls`) e input de velocidad (`#dash-speed-control`) se apilan verticalmente al 100% de ancho en celulares y regresan al formato horizontal en PC.
